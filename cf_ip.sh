...
get_history_ips(){
  ...
  case "$1" in
       4 ) grep -ioE "[0-9.]{7,}$" "${ANYCAST_SPEED_LOG}"
           ;;
       6 ) grep -ioE "[a-f0-9:]{7,}$" "${ANYCAST_SPEED_LOG}"
           ;;
       * ) grep -ioE "[a-f0-9:.]{7,}$" "${ANYCAST_SPEED_LOG}"
           ;;
  esac
}
...
get_fast_ip(){
  ...
  anycast_speed_ips=$(get_history_ips ${ip_type})
...
test_current_speed(){
  ...
  log 2 "Test download use these ips:\n${ips}"
  st=$(speed_test "${ips}" "$1")
}

cmp_exist_host(){
  [ "${cmp_exist}" = "1" ] || return
  cs=$(test_current_speed "${url}" "${real_addr}" "${dns}")
  cs_4=$(echo "${cs}" | grep -oE ".*,[0-9.]{7,}$")
  cs_6=$(echo "${cs}" | grep -oE ".*,[a-fA-F0-9:]{7,}$")

  fast_res_4=$(printf "%s\n%s" "${fast_data_4}" "${cs_4}")
  fast_data_4=$(echo "${fast_res_4}" | sort -n -r | head -${fast_cnt})
  
  fast_res_6=$(printf "%s\n%s" "${fast_data_6}" "${cs_6}")
  fast_data_6=$(echo "${fast_res_6}" | sort -n -r | head -${fast_cnt})
}
...
main() {
  ...
  if [ "$re_conf" = "1" ];then
    ...
    if [ -n "${real_addr}" ];then
      test_current_speed "${url}" "${real_addr}" "${dns}"
    else
      test_current_speed "${url}" "" "${dns}"
    fi
    exit
  fi
